...
function resetGame() {
    if (game) {
        const wasBot = game.gameMode === 'bot';
...
function renderBoard() {
    ...
    if (game.gameMode === 'friend' && game.boardFlipped) {
    ...
    for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
            const actualRow = (game.gameMode === 'friend' && game.boardFlipped) ? 7 - r : r;
            const actualCol = (game.gameMode === 'friend' && game.boardFlipped) ? 7 - c : c;
